        ...
        <div className="flex gap-4 overflow-x-auto pb-2 -mx-5 px-5 snap-x snap-mandatory scrollbar-hide">
          {hotSignals.map((signal) => (
            ...
            >
              ...
              {/* Content */}
              <div className="p-4">
                <p className="text-gray-500 text-xs line-clamp-2 mb-3">{t(`cards.${signal.id}.subheadline`)}</p>
                <div className="flex items-center justify-between">
                  <span className="text-[11px] text-gray-400">{t(`cards.${signal.id}.author`)} · {signal.readTime}</span>
                  <button
          ...
          >
            ...
            {expandedCard === signal.id && (
              <div className="px-4 pb-4 border-t border-gray-100">
                <p className="text-gray-600 text-sm mt-3 mb-4">{t(`cards.${signal.id}.subheadline`)}</p>
                ...
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  <p className="text-gray-900 text-sm font-medium">{t(`cards.${signal.id}.takeaway`)}</p>
                </div>

                <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-100">
                  <span className="text-xs text-gray-400">{tCommon('by')} {t(`cards.${signal.id}.author`)}</span>
